 ...
 */
public class VmAllocationPolicyPSO extends VmAllocationPolicy {
...
for(PowerHostUtilizationHistory hos:hoslist){
	double loghis=hos.getAvailableMips()-vm.getMips();
	if(loghis<100){
		idx=(idx+1) % hoslist.size();
		while(!(hoslist.get(idx).getAvailableMips()>vm.getMips()+100)){
		idx=hostCounter++ % hoslist.size();
		}
	}else{
