...
import org.cloudbus.cloudsim.VmAllocationPolicyGA;
...
private static List<Cloudlet> dataset;
	...
	public static void main(String[] args) {
		...
		try {
			...
			GADatacenterBroker brokerGA = GAcreateBroker();
			int brokerIdGA = brokerGA.getId();
			...
			for(int i=0;i<VMNo;i+=2){
				// 	VM description
					...
					// create VM
					PowerVm vm = new PowerVm(i,brokerIdGA, mips, pesNumber, ram, bw, size, 0, vmm, new CloudletSchedulerDynamicWorkload(mips, pesNumber),1); //new CloudletSchedulerDynamicWorkload(mips, pesNumber));
				//	Vm vm = new Vm(i,brokerIdBBO, mips, pesNumber, ram, bw, size, vmm,new  CloudletSchedulerSpaceShared()); //new CloudletSchedulerDynamicWorkload(mips, pesNumber));
					...
					PowerVm vm1 = new PowerVm(i+1, brokerIdGA, mips1, pesNumber, ram1, bw, size,0 ,vmm, new CloudletSchedulerDynamicWorkload(mips, pesNumber),1); //new CloudletSchedulerDynamicWorkload(mips, pesNumber));
...
//	double poissonObs =  1;//
	...
	cloudlet0.setUserId(brokerIdGA);
	cloudlet0.setSubmissionTime(aa);
	cloudletList.add(cloudlet0);// add the cloudlet to the list
}*/
dataset=workload.createCloudLets(brokerIdGA);
for(int i=0;i<1000;i++){
	dataset.get(i).SetCloudletId(i);
	dataset.get(i).setUserId(brokerIdGA);
	cloudletList.add(dataset.get(i));
}
brokerGA.submitCloudletList(cloudletList);
			// Sixth step: Starts the simulation
		final double finishTime = CloudSim.startSimulation();
		List<Cloudlet> newList = brokerGA.getCloudletReceivedList();
			...
			System.out.println("Convergene Time is: "+ brokerGA.convergenceTime);;
			
			
			System.out.println(brokerGA.convergenceTime);
...
int i=0;
		...
		try {
			...
			datacenter = new Datacenter(name, characteristics, new VmAllocationPolicyGA(hostList), storageList, 0);
	 ...
	 */
	private static GADatacenterBroker GAcreateBroker() {
