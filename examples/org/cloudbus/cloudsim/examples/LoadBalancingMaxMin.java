 ...
 * cloudlet on it.
 */
public class LoadBalancingMaxMin {
	...
	public static void main(String[] args) {
		...
		try {
			...
			MaxMinDatacenterBroker brokerBBO = createBroker();
...
//	int mips = 300;
	...
	PowerVm vm1 = new PowerVm(i+1,brokerIdBBO, mips, pesNumber, ram, bw, size, 0, vmm, new CloudletSchedulerDynamicWorkload(mips, pesNumber),1); //new CloudletSchedulerDynamicWorkload(mips, pesNumber));
	// add the VM to the vmList
	vmlist.add(vm1);
...
brokerBBO.submitCloudletList(cloudletList);
		...
		List<Cloudlet> newList = brokerBBO.getCloudletReceivedList();
			CloudSim.stopSimulation();
			//Final step: Print results when simulation is over
	
			printCloudletList(newList);

			//System.out.println(brokerBBO.convergenceTime);
...
int i=0;
	...
	hostList.add(
		...
		try {
			datacenter = new Datacenter(name, characteristics, new VmAllocationPolicyMaxMin(hostList), storageList, 0);
	 ...
	 */
	private static MaxMinDatacenterBroker createBroker() {
		MaxMinDatacenterBroker broker = null;
		try {
			broker = new MaxMinDatacenterBroker("Broker");
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
