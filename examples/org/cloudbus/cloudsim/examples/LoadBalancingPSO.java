...
import org.cloudbus.cloudsim.GADatacenterBroker;
import org.cloudbus.cloudsim.Host;
import org.cloudbus.cloudsim.HostDynamicWorkload;
import org.cloudbus.cloudsim.HostStateHistoryEntry;
import org.cloudbus.cloudsim.Log;
import org.cloudbus.cloudsim.PSODatacenterBroker;
...
import org.cloudbus.cloudsim.VmAllocationPolicyPSO;
 ...
 * cloudlet on it.
 */
public class LoadBalancingPSO {

	public static int VMNo=200;
	...
	public static void main(String[] args) {
		...
		try {
			...
			@SuppressWarnings("unused")
			Datacenter datacenter1 = createDatacenter("Datacenter_1",1,3,-4,30000);
			@SuppressWarnings("unused")
			Datacenter datacenter2 = createDatacenter("Datacenter_2",1,12,3.5,22000);
			List<Datacenter> listdc=new ArrayList<Datacenter>();
			listdc.add(datacenter1);
			listdc.add(datacenter2);
			@SuppressWarnings("unused")
			Datacenter datacenter3 = createDatacenter("Datacenter_3",4,2,9,8000);
			listdc.add(datacenter3);
			Datacenter datacenter4 = createDatacenter("Datacenter_4",2,4,7,10000);
			listdc.add(datacenter4);
			// Third step: Create Broker
			PSODatacenterBroker brokerPSO = PSOcreateBroker();
			int brokerIdPSO = brokerPSO.getId();
			...
			for(int i=0;i<VMNo;i+=2){
				// 	VM description
					...
					// create VM
					PowerVm vm = new PowerVm(i,brokerIdPSO, mips, pesNumber, ram, bw, size, 0, vmm, new CloudletSchedulerDynamicWorkload(mips, pesNumber),1); //new CloudletSchedulerDynamicWorkload(mips, pesNumber));
				//	Vm vm = new Vm(i,brokerIdBBO, mips, pesNumber, ram, bw, size, vmm,new  CloudletSchedulerSpaceShared()); //new CloudletSchedulerDynamicWorkload(mips, pesNumber));
					...
					PowerVm vm1 = new PowerVm(i+1, brokerIdPSO, mips1, pesNumber, ram1, bw, size,0 ,vmm, new CloudletSchedulerDynamicWorkload(mips, pesNumber),1); //new CloudletSchedulerDynamicWorkload(mips, pesNumber));
...
//	double poissonObs =  1;//
	...
	cloudlet0.setUserId(brokerIdPSO);
	cloudlet0.setSubmissionTime(aa);
	cloudletList.add(cloudlet0);// add the cloudlet to the list
}*/
dataset=workload.createCloudLets(brokerIdPSO);
for(int i=0;i<1000;i++){
	dataset.get(i).SetCloudletId(i);
	dataset.get(i).setUserId(brokerIdPSO);
	cloudletList.add(dataset.get(i));
}
brokerPSO.submitCloudletList(cloudletList);
			// Sixth step: Starts the simulation
		final double finishTime = CloudSim.startSimulation();
		List<Cloudlet> newList = brokerPSO.getCloudletReceivedList();
			CloudSim.stopSimulation();
			//Final step: Print results when simulation is over
	
			printCloudletList(newList);

			System.out.println(brokerPSO.convergenceTime);
		//	System.out.println(datacenter1.getVmList().get(0).getRam());
			...
			System.out.println("Convergene Time is: "+ brokerPSO.convergenceTime);
	 ...
	 */
	private static Datacenter createDatacenter(String name,int hosts,int pes,double timezone,int MIPS) {
...
int i=0;
	...
	hostList.add(
		...
		//	datacenter = new Datacenter(name, characteristics, new VmAllocationPolicySimple(hostList), storageList, 0);
			datacenter = new Datacenter(name, characteristics, new VmAllocationPolicyPSO(hostList), storageList,0);
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 */
	private static PSODatacenterBroker PSOcreateBroker() {
		...
		for (int i = 0; i < size; i++) {
			...
			if (cloudlet.getCloudletStatus() == Cloudlet.SUCCESS) {
				...
				Log.printLine(indent + indent + cloudlet.getResourceId()
						...
						+ indent + indent
						+ dft.format(cloudlet.getFinishTime())+ indent +i);
			}
		}
	}
    public static void printUtilization(Datacenter datacenter1,Datacenter datacenter2,Datacenter datacenter3,Datacenter datacenter4){
		for(HostDynamicWorkload h:datacenter1.getHostList()){
			int totalipsused=0;
		//	for(HostStateHistoryEntry state:h.getStateHistory())
			//	if(state.isActive()){
		//			totalipsused+=state.getAllocatedMips();//.getAllocatedMips();
			//	}
		//	System.out.println("DC3: Host: "+h.getId() +"    Util: "+totalipsused/h.getTotalMips());

			System.out.println("DC1: Host: "+h.getId() +"    Util: "+h.getUtilizationOfCpu());
		}
		
		for(HostDynamicWorkload h:datacenter2.getHostList()){
			int totalipsused=0;
		//	for(HostStateHistoryEntry state:h.getStateHistory()){
		//		if(state.isActive()){
		//			totalipsused+=state.getAllocatedMips();
		//		}
		//	}
				
			System.out.println("DC2: Host: "+h.getId() +"    Util: "+h.getUtilizationOfCpu());
		
		}
		
		for(HostDynamicWorkload h:datacenter3.getHostList()){
			int totalipsused=0;
		//	for(HostStateHistoryEntry state:h.getStateHistory())
		//		if(state.isActive()){
		//			totalipsused+=state.getAllocatedMips();
		//		}
			System.out.println("DC3: Host: "+h.getId() +"     Util: "+h.getUtilizationOfCpu());
	
		}
		
		for(HostDynamicWorkload h:datacenter4.getHostList()){
			int totalipsused=0;
		//	for(HostStateHistoryEntry state:h.getStateHistory())
		//		if(state.isActive()){
		//			totalipsused+=state.getAllocatedMips();
		//		}
			System.out.println("DC4: Host: "+h.getId() +"    Util: "+h.getUtilizationOfCpu());
